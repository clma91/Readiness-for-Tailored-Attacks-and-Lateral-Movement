...
Describe 'Mock EventLogs'{
  $logs = New-Object System.Collections.ArrayList
  $logs = Import-Csv ".\myeventlogs.csv"
  It 'mocks eventlogs'{
    $events += Mock Get-EventLog {return $logs}
    $time = Get-Date {return New-Object datetime(2015, 4, 20)}
    #$events.Count | Should -Be 109
    $time.Ticks | Should -Be 1062
  }
}