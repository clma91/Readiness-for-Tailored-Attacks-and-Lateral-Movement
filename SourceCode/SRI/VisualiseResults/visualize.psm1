...
function visualizeAll($importFolder, $exportFolder) {
    $pdf = OpenPDF $exportFolder
    writeAuditPolicies $importFolder
    writeEventLogs $importFolder
    $pdf.Close()
}

function visualizeAuditPolicies($importFolder, $exportFolder) {
    $pdf = OpenPDF $exportFolder
    writeAuditPolicies $importFolder
    $pdf.Close()
}

function visualizeEventLogs($importFolder, $exportFolder){
    $pdf = OpenPDF $exportFolder
    writeEventLogs $importFolder
    $pdf.Close()
}

function writeAuditPolicies($importFolder, $exportFolder) {
    ...
    foreach ($audit in $myaudits) {
        ...
        if ($audit.InnerXml -eq $checkaudit) {
            ...
            $table.AddCell($cell) | Out-Null
            $cell = New-Object iTextSharp.text.pdf.PdfPCell($audit.InnerXml)
            $cell.BackgroundColor = New-Object iTextSharp.text.BaseColor(253, 106, 2)
            $table.AddCell($cell) | Out-Null
