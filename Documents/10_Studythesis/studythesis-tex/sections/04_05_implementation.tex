...
In this script we implemented the ''\nameref{UC05}'', here the description how we have done it.
\subsubsection{Result} \label{resultVisualize}
The script ''\nameref{UC05}'' runs as follows:
\begin{itemize}
    \item Create PDF-document at given folder and ''open'' it
    ...
    \item Import the found EventLogs and check if the important EventIDs, according to JPCERT, are found
...
We then use the function \lstinline|WriteAuditPolicies| to compare the found audit policies and display the incorrectly.
\begin{lstlisting}[caption=Function WriteAuditPolicies, language=PowerShell]
