...
The resulting XML-files from AnalyseEvents() and AnalyseAuditPolicy() are gathered and compared with a checklist, which is based on the recommendation from JPCERT/CCs study (see \ref{JPCertStudy} \nameref{JPCertStudy}). Incorrect or missing configuration is highlighted in red, correct configurations in green. The event logs are listed as missing or present.
